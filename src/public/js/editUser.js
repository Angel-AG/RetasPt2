async function editUserSubmit(e) {
  e.preventDefault();
  ...
  try {
    ...
    const response = await axios.put('/user/', inputs);
    sessionStorage.setItem('user', response.data.user);

    let myModal = new bootstrap.Modal(document.getElementById('good-modal'), {
      keyboard: false,
    });
    myModal.show();
    // location.assign('/');
    ...
    console.error(error);
    let myModal = new bootstrap.Modal(document.getElementById('bad-modal'), {
      keyboard: false,
    });
    myModal.show();
    // alert(error.msg);
